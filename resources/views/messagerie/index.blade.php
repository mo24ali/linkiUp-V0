    ...
    <div class="grid grid-cols-12 gap-6 relative">
        ...
        <aside id="chat-sidebar" class="col-span-12 md:col-span-4 lg:col-span-3 space-y-6">
            ...
            <div class="glass-card p-4">
                <h3 class="font-bold text-[#e7e9ea] mb-4 flex items-center">
                    <svg class="w-5 h-5 mr-2 text-[#1d9bf0]" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M19.25 3.018H4.75C3.233 3.018 2 4.252 2 5.77v12.495c0 1.518 1.233 2.753 2.75 2.753h14.5c1.517 0 2.75-1.235 2.75-2.753V5.77c0-1.518-1.233-2.752-2.75-2.752zm-14.5 1.5h14.5c.69 0 1.25.56 1.25 1.252v.918l-8.5 5.11-8.5-5.11v-.918c0-.692.56-1.252 1.25-1.252zm14.5 14.998H4.75c-.69 0-1.25-.56-1.25-1.252V8.516l8.5 5.11 8.5-5.11v9.748c0 .692-.56 1.252-1.25 1.252z"/>
                    </svg>
                    Conversations
                </h3>
                ...
                <ul id="conversations-list" class="space-y-2">
                        ...
                        <li onclick="selectConversation({{ $conversation->id }}, {{ $otherUser->id }}, '{{ $otherUser->name }}')"
                            ...
                            <div class="flex items-center gap-3">
                                <div class="relative">
                                    ...
                                    @else
                                        <div class="w-12 h-12 rounded-full bg-gradient-to-r from-[#1d9bf0] to-[#f91880] flex items-center justify-center text-white font-bold text-lg border-2 border-[#2f3336] group-hover:border-[#1d9bf0] transition-colors">
                                            {{ substr($otherUser->name, 0, 1) }}
                                        </div>
                                    @endif
                                    <span class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full ring-2 ring-[#202327]"></span>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex justify-between items-baseline">
                                        <p class="font-bold text-[#e7e9ea] truncate group-hover:text-[#1d9bf0] transition-colors">
                                            {{ $otherUser->name }}
                                        </p>
                    ...
                    @empty
                        <div class="text-center py-8">
                            <svg class="w-12 h-12 text-[#71767b] mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="glass-card p-4">
                <h3 class="font-bold text-[#e7e9ea] mb-4 flex items-center">
                    <svg class="w-5 h-5 mr-2 text-[#1d9bf0]" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm0 18c-4.411 0-8-3.589-8-8s3.589-8 8-8 8 3.589 8 8-3.589 8-8 8zm-1-13h2v6h-2V7zm0 8h2v2h-2v-2z"/>
                    </svg>
                    Start New Chat
                </h3>
                        ...
                        <li onclick="startNewChat({{ $user->id }}, '{{ $user->name }}')"
                            ...
                            <div class="flex items-center gap-3">
                                <div class="relative">
                                    ...
                                    @else
                                        <div class="w-10 h-10 rounded-full bg-gradient-to-r from-[#1d9bf0] to-[#f91880] flex items-center justify-center text-white font-bold border-2 border-[#2f3336] group-hover:border-[#1d9bf0] transition-colors">
                                            {{ substr($user->name, 0, 1) }}
                                        </div>
                                    @endif
                                    <span class="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full ring-2 ring-[#202327]"></span>
                                </div>
        ...
        <main id="chat-main" class="col-span-12 md:col-span-8 lg:col-span-6 hidden md:block">
            <div class="glass-card p-0 flex flex-col h-[calc(100vh-180px)] relative">
                ...
                <!-- Chat Header -->
                <div id="chat-header" class="p-5 border-b border-[#2f3336] sticky top-0 z-10 bg-black/80 backdrop-blur-md hidden">
                    <div class="flex items-center gap-3">
                        <button onclick="showSidebar()" class="md:hidden mr-1 text-[#e7e9ea] hover:bg-[#202327] p-2 rounded-full transition-colors group">
                            <svg class="w-5 h-5 group-hover:text-[#1d9bf0]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                            </svg>
                        </button>
                        <div class="relative">
                            <div id="chat-avatar" class="w-12 h-12 rounded-full bg-gradient-to-r from-[#1d9bf0] to-[#f91880] flex items-center justify-center text-white font-bold text-lg border-2 border-[#2f3336]">
                ...
                <!-- Messages Container -->
                <div id="messages"
                    class="flex-1 overflow-y-auto p-6 space-y-4 bg-[#15181c] hidden">
        ...
        <!-- RIGHT SIDEBAR - Empty but kept for layout consistency -->
        <aside class="hidden lg:block lg:col-span-3">
            <div class="glass-card p-6 sticky top-24">
                <h3 class="font-bold text-lg text-[#e7e9ea] mb-4 flex items-center">
                    <svg class="w-5 h-5 mr-2 text-[#1d9bf0]" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-.5-13v3h-3v2h3v3h2v-3h3v-2h-3V7h-2z"/>
                    </svg>
                    Chat Tips
                </h3>
                <div class="space-y-4 text-sm">
                    <div class="text-[#71767b]">
                        <span class="font-bold text-[#e7e9ea] block mb-1">💬 Real-time messaging</span>
                        <p>Messages appear instantly when both users are online.</p>
                    </div>
                    <div class="text-[#71767b]">
                        <span class="font-bold text-[#e7e9ea] block mb-1">🔵 Online status</span>
                        <p>Green dot indicates active users ready to chat.</p>
                    </div>
                    <div class="text-[#71767b]">
                        <span class="font-bold text-[#e7e9ea] block mb-1">✨ Quick start</span>
                        <p>Click any online user to begin a new conversation.</p>
                    </div>
                </div>
            </div>
        </aside>
